...
public class Stock extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("pageload") != null) {
			if (request.getParameter("pageload").equalsIgnoreCase("getStockList")) {
				StockDB objDb = new StockDB();
				ArrayList<StockMaterial> stockMaterialsList= (ArrayList<StockMaterial>) objDb.getStockMaterials();
				response.setContentType("application/json");
				response.setCharacterEncoding("UTF-8");
				new Gson().toJson(stockMaterialsList, response.getWriter());
			}else if (request.getParameter("pageload").equalsIgnoreCase("getStock")) {
				StockDB objDb = new StockDB();
